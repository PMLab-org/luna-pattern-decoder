...
const faqs = [
  ...
  },
  {
    ...
    a: "Luna is free to start. You can also join as a founding member for US$75/year and lock in that price forever.",
  },
];
                ...
                {"outcomes" in s && s.outcomes ? (
                  <p className="mt-3 text-muted-foreground">
                    {s.outcomes.map((o, i) => (
                      <span key={o.label}>
                        {i > 0 && " "}
                        <span className="font-semibold text-foreground">{o.label}:</span> {o.body}
                      </span>
                    ))}
                  </p>
                ) : (
